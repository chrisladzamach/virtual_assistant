...
Uso el modelo 0613, pero usted, puede usar un poco de
prompt engineering si quiere usar otro modelo
"""
class LLM():
  def __init__(self):
    pass
  ...
  def process_functions(self, text):
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
                # Si no le gusta que le hable feo, cambie aqui la descripcion
      ...
      if message.get("function_call"):
          ...
          args = message.to_dict()['function_call']['arguments'] # ¿Con qué datos?
  ...
  def process_response(self, text, message, function_name, function_response):
      response = openai.ChatCompletion.create(
          model="gpt-3.5-turbo-0613",
          messages=[
              # Aqui tambien puede cambiar el cómo se comporta
              {"role": "system", "content": "Eres un asistente malhablado"},
              {"role": "user", "content": text},
              message,
              {
